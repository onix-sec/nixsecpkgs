...
def main():
    ...
    parser.add_argument(
        "-i", "--input", type=str, help="File containing raw shellcode."
    )
    parser.add_argument(
        "-o", "--output", type=str, help="Output file. Defaults to 'generated.c.'"
    )
    ...
    """
    translated_shellcode_gen = (
        '"{}"'.format(english_array[int(byte, 16)]) for byte in shellcode.split(",")
    )
    translated_shellcode = "unsigned char* translated_shellcode[XXX] = { " + ",".join(
        translated_shellcode_gen
    )
